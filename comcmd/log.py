"""Structured logging for Company Command library code.
...
def configure(level: str | None = None) -> None:
    ...
    if _CONFIGURED:
        return
    lvl = (level or os.environ.get("COMCMD_LOG_LEVEL", "INFO")).upper()
    logging.basicConfig(
    ...
    logging.getLogger("dbos").setLevel(
        getattr(logging, os.environ.get("COMCMD_DBOS_LOG_LEVEL", "WARNING"), logging.WARNING))
    _CONFIGURED = True


def get_logger(name: str) -> logging.Logger:
    return logging.getLogger(name if name.startswith("comcmd") else f"comcmd.{name}")
