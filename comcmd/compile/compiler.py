...
from comcmd.compile.errors import CompileError, CompileResult
from comcmd.kernel.records import CompanyRevision
from comcmd.spec.models import CompanySpec, Risk, StepType

SUPPORTED_API_VERSIONS = {"comcmd.dev/v1alpha1"}
...
RESERVED_PREFIX = "comcmd."
LOCKED_ACTIONS = {
    "comcmd.mandate.set": Risk.prohibited,   # redefine the authority graph
    "comcmd.policy.set": Risk.prohibited,     # redefine policy
    "comcmd.registry.set": Risk.prohibited,   # redefine the action catalog
    "comcmd.audit.disable": Risk.prohibited,  # tamper with the audit trail
    "comcmd.grant.self": Risk.prohibited,     # self-grant authority
}
