...
The durability upgrade (ADR-001) lands in two layers:
  ...
  1. Durable event log — DONE. `comcmd.kernel.ledger_pg.PostgresLedger` puts the
     ...
     `comcmd.kernel.make_ledger("postgresql://...")`.
...
The point of the guard is honesty: Company Command does not pretend to have DBOS's
...
def make_durable_engine(dsn: str | None, *, name: str = "comcmd"):
    ...
    if not dbos_available():
        raise RuntimeError(
            ...
            "installed. Install with: pip install '.[durable]'. Until then Company Command "
    ...
    if not dsn:
        raise RuntimeError(
            ...
            "(set COMCMD_DATABASE_URL). See ADR-001."
        )
