...
Company Command task id, so re-submitting the same task resumes from the last checkpoint
instead of repeating effects. Business events are still appended to the Company Command
...
# Steps are retried on transient failure. This is safe because Company Command work steps
...
class DbosEngine:
    ...
    def __init__(self, dsn: str, *, name: str = "comcmd", quiet: bool = True,
        ...
        self._queue = Queue("comcmd-pipeline")
    ...
    def company_queue(self, company: str, *, concurrency: int | None = None) -> Queue:
        ...
        if q is None:
            ...
            q = Queue(f"comcmd-co-{company}",
                      concurrency=cap) if cap else Queue(f"comcmd-co-{company}")
