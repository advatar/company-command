...
from comcmd.gateway.approvals import ApprovalStore, parse_ttl
from comcmd.gateway.intents import ActionIntent
from comcmd.gateway.policy import (
    APPROVAL_TIERS,
    AUTO_TIERS,
    Bounds,
    DENY_TIERS,
...
from comcmd.gateway.verifier import ApprovalVerifier, DenyByDefaultVerifier
...
from comcmd.log import get_logger
from comcmd.spec.models import Action
from comcmd.telemetry import Telemetry, get_telemetry
...
class Gateway:
    def __init__(
        ...
        self._significant: dict[str, dict] = {}
    ...
    def pending_approvals(self) -> list[dict]:
        return [self._approval_view(pa, self._significant.get(pa.action_digest))
                for pa in self._approvals.pending()]
    ...
    def _open_approval(self, intent: ActionIntent, action: Action, tier: Tier,
        ...
        pa = self._approvals.open(action_digest=intent.action_digest, tier=tier.value,
                                  ...
                                  ttl_seconds=ttl)
        request = self._public_request(pa)
        request["significant"] = intent.canonical()
        self._significant[intent.action_digest] = request["significant"]
        ...
        return GateOutcome("require_approval", tier, reason,
                           approval_request=self._approval_view(pa, intent.canonical()))
    ...
    def _approval_view(self, pa, significant: dict | None = None) -> dict:
        view = self._public_request(pa)
