...
def test_durable_when_database_url_set():
    s = Settings.from_env({"COMCMD_DATABASE_URL": "postgresql://x/y"})
    assert s.durable is True
    assert s.effective_ledger_url == "postgresql://x/y"


def test_ledger_url_overrides():
    s = Settings.from_env({"COMCMD_DATABASE_URL": "postgresql://a/b",
                           "COMCMD_LEDGER_URL": "postgresql://c/d"})
    assert s.effective_ledger_url == "postgresql://c/d"


def test_model_map_parsing():
    s = Settings.from_env({"COMCMD_MODEL_MAP": "planner-high=llama-3.3, fast=qwen"})
